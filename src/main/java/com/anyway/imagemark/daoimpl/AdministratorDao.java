...
import com.anyway.imagemark.utils.MD5;
...
/*	public Administrator getAdministratorByNameOrMail(String adminNameOrMail) {
		System.out.println("AdministratorDao(28)"+adminNameOrMail);
	...
	public Administrator getAdministratorByNameOrMail(String adminNameOrMail) {
		System.out.println("AdministratorDao(28)"+adminNameOrMail);	
		DBObject query = mHelper.createQuery("adminName", adminNameOrMail);
...
/*		BasicDBObject query = new BasicDBObject();
		...
		// 数据丢失?
*/		
		DBObject query = mHelper.createQuery("adminName", name);
		query.put("status", ToolConstants.VALID_INT);
		DBCursor cursor = collection.find(query);
		if (cursor.hasNext()) {
			log.info("update");
			BasicDBObject obj = new BasicDBObject();
			obj.append("$set",mHelper.createQuery("adminPassword", newPwd));
	...
	//根据用户名查找数据库查找该用户
	public Administrator isUserExist(String name){
		BasicDBObject query = new BasicDBObject();
		query.append("adminName", name);
		DBCursor cursor = collection.find(query);
		...
		if (cursor.hasNext()){
			DBObject obj = cursor.next();
			administrator = gson.fromJson(obj.toString(),Administrator.class);
		}
		return administrator;
	}
	
	//判断checkCode是否有效
	public boolean  isCheckCodeValid(Administrator administrator,String checkCode){
		String randomCode = administrator.getRandomCode();
		System.out.println("AdministratorDao(442) "+administrator.getAdminName());
		String str = administrator.getAdminName()+":"+randomCode;
		System.out.println("AdministratorDao(444)"+str);
		String ran_md5=MD5.getMD5ofStr(str);
		System.out.println("AdministratorDao(446)   randomCode:"+randomCode);
		System.out.println("AdministratorDao(447)   checkCode:"+checkCode);
		System.out.println("AdministratorDao(448)   ran_md5:"+ran_md5);
		
		if(checkCode.equals(ran_md5)){
			System.out.println("*************AdministratorDao449***************");
			return true;
		}
		System.out.println("*************AdministratorDao451***************");
		return false;
	}
    ...
    /** 
     *   生成邮件链接
	 *  用户名和随机号组合后，通过md5加密后的16进制格式的字符串 
     */   
    public  String generateActiveLink(String url,String username,String randomCode){  	
    	String str = username+":"+randomCode;
    	System.out.println("****************AdministratorDao466*************str="+str);
    	System.out.println("****************AdministratorDao467*************"+MD5.getMD5ofStr(str));
    	 return url+"?username="+ username + "&checkCode"  + "=" +MD5.getMD5ofStr(username+":"+randomCode);  
    }
	//********************************************************KG**********************************************
}
